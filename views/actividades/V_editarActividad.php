...
<main>
  ...
  <div class="container d-flex justify-content-center mb-3">
    <form data-bitwarden-watching="1" action="index.php?c=actividad&a=actualizar" method="POST">
      <fieldset>
        <legend class="mt-4 text-center text-primary">
          <h1><strong>Editando actividad</strong></h1>
        </legend>
        <fieldset class="form-group">
          <legend>Detalles</legend>
          <input name="asignatura" type="hidden" class="form-control" id="asignatura" value="<?php echo $_GET['id']; ?>">
          <input name="actividad" type="hidden" class="form-control" id="actividad" value="<?php echo $_GET['f']; ?>">
          <div class="form-group">
            <label for="nombre" class="form-label mt-2">Nombre de la actividad</label>
            <input name="nombre" type="text" class="form-control" id="nombre" placeholder="Ej. Actividad No. 0" autocomplete="off" value="<?php echo $datos['nombre_act'] ?>">
          </div>
          ...
          <div class="form-group">
            ...
            <select class="form-select" id="tipo" name="tipo" value=" ">
              ...
              <option <?php if ($datos['tipo'] == '1') echo "selected='selected'"; ?>>1. Trabajo</option>
              <option <?php if ($datos['tipo'] == '2') echo "selected='selected'"; ?>>2. Tarea</option>
              <option <?php if ($datos['tipo'] == '3') echo "selected='selected'"; ?>>3. Examen</option>
            </select>
          </div>

          <div class="form-group">
            <label for="fecha" class="form-label mt-3">Fecha de la actividad</label>
            <input name="fecha" type="date" class="form-control" id="fecha" value="<?php echo $datos['fecha_actividad'] ?>">

            <div class="form-group">
              <label for="descripcion" class="form-label mt-3">Descripcion</label>
              <textarea class="form-control" id="descripcion" name="descripcion" rows="3" data-dl-input-translation="true" placeholder="Ej. Requiere reforzar lectura"><?php echo $datos['descripcion'] ?></textarea>
